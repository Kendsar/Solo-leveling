...
export function DayCard({ dayData, delay = 0 }: DayCardProps) {
  ...
  const isToday = new Date(date).toDateString() === todayStr;
        ...
        <div className="relative p-5 flex flex-col h-full z-10">
          ...
          <div className="mb-4 flex items-center gap-2">
            {isEditingTitle ? (
              ...
              <>
                <h3 className="text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-100 to-cyan-400 flex-1">
                  {localTitle}
                </h3>
                {isToday && <Pencil className="w-4 h-4 text-cyan-400 cursor-pointer" onClick={() => setIsEditingTitle(true)} />}
              </>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2 mb-6">
            {localExercises.length === 0 ? (
              ...
              <div key={idx} className="flex items-start gap-3 bg-slate-950/40 p-2.5 rounded border border-cyan-500/10 hover:border-cyan-500/30 transition-colors cursor-pointer">
                <div className="mt-0.5" onClick={() => toggleTaskCompletion(idx)}>
                  {ex.completed ? <CheckSquare className="w-4 h-4 text-emerald-400" /> : <Square className="w-4 h-4 text-cyan-400/40" />}
                </div>
                <div className="flex flex-col flex-1">
                  {editingIndex === idx ? (
                    ...
                    <span className={`${ex.completed ? 'line-through text-emerald-100/80' : 'text-cyan-50'} text-sm`}>
                      {ex.name}
                    </span>
                  )}
                  {ex.details && <span className="text-[10px] text-cyan-400/60 font-mono mt-0.5">{ex.details}</span>}
                </div>
                {isToday && editingIndex !== idx && (
                  <Pencil className="w-3.5 h-3.5 text-cyan-400 cursor-pointer mt-0.5" onClick={() => { setEditingIndex(idx); setEditingValue(ex.name); }} />
                )}
              </div>
            ))}
          </div>
          ...
          {localExercises.length > 0 && (
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs font-mono flex items-center gap-1 text-cyan-300">{completionRate}% {completionRate === 100 && <Check className="w-3.5 h-3.5 text-emerald-400" />}</span>
              </div>
                ...
                {isAddingTask ? (
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); setIsAddingTask(false); }} className="text-slate-500 hover:text-red-400 transition-colors">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      <input
                        ...
                        className="w-1/3 bg-slate-900 border border-slate-700 rounded px-2 py-1.5 text-[11px] text-cyan-50"
                      />
                      <button onClick={() => setIsLbs(!isLbs)} className="flex items-center bg-slate-900 border border-slate-700 rounded p-0.5 text-[9px] font-bold">
                        <span className={!isLbs ? "bg-cyan-500/20 text-cyan-300 px-1" : "text-slate-500 px-1"}>kg</span>
                        <span className={isLbs ? "bg-cyan-500/20 text-cyan-300 px-1" : "text-slate-500 px-1"}>lbs</span>
                      </button>
                    </div>
                    ...
                    >
                      <Save className="w-3.5 h-3.5 group-hover:scale-110 transition-transform" /> Save
                    </button>
                  </motion.div>
                ) : (
                  <motion.button
